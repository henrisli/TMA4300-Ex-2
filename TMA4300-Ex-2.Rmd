...
From figure \ref{fig:acfu} we see that the three components of $\mathbf{u}$ show varying degree of autocorrelation. It takes roughly $50$ iterations to reduce the autocorrelation of the three components to non-significance. Even so, the components are less correlated than $\kappa_v$.
...
```
For the three components of $\mathbf{v}$ in figure \ref{fig:comp3v} we see that the improved INLA coincides with the original INLA. Also, the marginal densities from INLA perfectly coincides with the histograms of the MCMC samples.
...
We note that even though INLA gives somewhat different estimates for $\mathbf{u}$, the INLA marginals of the three components of $\boldsymbol \eta = \mathbf{u} + \mathbf{v}$ perfectly overlaps with the MCMC histogram, see figure \ref{fig:comp3eta}.

```{r, echo = F, eval = T}
INLA.time = round(result$cpu.used[4],1)
```
We note that INLA and MCMC in general produce results that are at least qualitatively comparable. For $\kappa_u$, $\kappa_v$ and $\eta$, the resulting marginal densities perfectly overlap. For the spatial effect $\mathbf{u}$ we observe some difference. Also worth noting is that the MCMC sampler uses `r round(sum(times)/60)` minutes, while INLA uses `r INLA.time` seconds to run. Because they yield very similar results, we see that INLA is vastly superior time consumption is a significant restriction. 
