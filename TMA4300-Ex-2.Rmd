...
```{r, echo = F, eval = T, out.width = "33%", fig.align = "center", fig.cap = "\\label{fig:acfeta1}Autocorrelation plot for component 157 of $\\boldsymbol \\eta$"}
df7 = data.frame(eta = eta[,random_indexes])
acf(eta[,random_indexes[1]], lag.max = 200, main = paste0("ACF of eta[",random_indexes[1],"]"))
```
```{r, echo = F, eval = T, out.width = "33%", fig.align = "center", fig.cap = "\\label{fig:acfeta2}Autocorrelation plot for component 222 of $\\boldsymbol \\eta$"}
acf(eta[,random_indexes[2]], lag.max = 200, main = paste0("ACF of eta[",random_indexes[2],"]"))
```
```{r, echo = F, eval = T, out.width = "33%", fig.align = "center", fig.cap = "\\label{fig:acfeta3}Autocorrelation plot for component 429 of $\\boldsymbol \\eta$"}
acf(eta[,random_indexes[3]], lag.max = 200, main = paste0("ACF of eta[",random_indexes[3],"]"))
```

From figures \ref{fig:acfeta1}-\ref{fig:acfeta3} we see that the three components of $\boldsymbol \eta$ show varying degree of autocorrelation. As it takes roughly $100$ iterations to reduce the autocorrelation to non-significance we observe that the three components are more autocorrelated than the components of $\mathbf{u}$, but still less correlated than $\kappa_v$.
