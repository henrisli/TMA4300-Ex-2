...
```{r, echo = F, eval = T, out.width = "50%", fig.cap = "\\label{fig:tracekappau}Trace plot of kappa_u. All iterations plotted to the left, first 500 discarded plotted to the right."}
df1 = data.frame(kappa_u = c(kappau, kappau[501:N_iter]), N = c(1:N_iter,501:N_iter), type = c(rep("All iterations",N_iter), rep("First 500 discarded",N_iter-500)))
ggplot(df1, aes(x = N, y = kappa_u)) + geom_point() + facet_wrap(.~type, scales = "free")
```
...
```
```{r, echo = F, eval = T, out.width = "50%", fig.cap = "\\label{fig:tracekappau}Trace plot of kappa_v. All iterations plotted to the left, first 500 discarded plotted to the right."}
df1 = data.frame(kappa_v = c(kappav, kappav[501:N_iter]), N = c(1:N_iter,501:N_iter), type = c(rep("All iterations",N_iter), rep("First 500 discarded",N_iter-500)))
ggplot(df1, aes(x = N, y = kappa_v)) + geom_point() + facet_wrap(.~type, scales = "free")
```
...
cat("pvals:", 2*(1-pnorm(abs(gw$z))))
```
$p$-values of `r cat(round(2*(1-pnorm(abs(gw$z))),2))` indicate that the chain converges during the first $10\%$ iterations for only component 7 and 424, and not for component 257.
...
cat("pvals:", 2*(1-pnorm(abs(gw$z))))
```
$p$-values of `r cat(round(2*(1-pnorm(abs(gw$z))),2))` indicate that the chain converges during the first $10\%$ iterations for only component 7, and not for component 257 and 424. ENDRE ALLE DISSE BASERT PÅ NYE ITERASJONER!!!!!!!!!!!!!!!
...
Because the parameters are correlated, we get high autocorrelation in the Markov chain, and hence a low value for the ESS. One way to improve this is by blocking. Blocking is done by simulating the variables together, and accepting or rejecting all of the new proposed variables simultaneously.
...
```{r, echo = F, eval = T, out.width = "50%", fig.cap = "\\label{fig:medianexpu} Posterior median of exp(u) for all regions in Germany", fig.align = "center"}
post_median_u = apply(exp(u[100:N_iter,]), 2, median)
...
# WHY DO WE NEED CONSTR = F HERE
#formula <- Y~f(region,model="besag", graph = g, hyper=list(prec=kappauprior), constr=F) + 
#            f(region2, model="iid", hyper=list(prec=kappavprior))

formula <- Y~f(dfinla$region,model="besag", graph = g, hyper=list(prec=kappauprior)) + 
            f(dfinla$region2, model="iid", hyper=list(prec=kappavprior))


# Oral$Y = Oral$E * eta
res <- inla(formula,family="poisson", E = dfinla$E, data=dfinla)

# Look at results
#res$summary.random$`dfinla|S|region`

res2 = inla.hyperpar(res)

germany.plot(exp(res$summary.random$`dfinla|S|region`$mean), col=col, legend=TRUE)
germany.plot(post_median_u, col=col, legend=TRUE)


germany.plot( exp(res$summary.random$`dfinla|S|region`$mean) - post_median_u, col=col, legend=TRUE)
```

```{r, echo=T, eval=T}
exp(res$summary.random$`dfinla|S|region`$mean[random_indexes])
post_median_u[random_indexes]
```

```{r, echo=T, eval=T}
df60 = data.frame(res2$marginals.hyperpar$`Precision for dfinla|S|region`)
df61 = data.frame(x = kappau[100:length(kappau)])

ggplot() + geom_histogram(data=df61, aes(x = x, y = ..density.., col="MCMC"), bins=100) + geom_line(data=df60, aes(x = x, y = y, col="INLA")) + xlim(0, 40)
```

```{r, echo=T, eval=T}
df60 = data.frame(res2$marginals.hyperpar$`Precision for dfinla|S|region2`)
df61 = data.frame(x = kappav[100:length(kappav)])

ggplot() + geom_histogram(data=df61, aes(x = x, y = ..density.., col="MCMC"), bins=100) + geom_line(data=df60, aes(x = x, y = y, col="INLA")) + xlim(0, 500)
```
