...
We observed that the inclusion of smoking in the model produces a reduction in loglikelihood that outweighs the parameter penalty. However, we saw that the reduction for the random walk model and linear model were nearly equal. This means that the reduction in loglikelihood permitted by the random-walk model is outweighed by the penalty on the number of parameters introduced. Thus, using DIC, we can argue that the linear and random-walk models are equally good. 

We observe that the credible interval for the random walk model does not contain the zero-hypothesis for all values of smoking but is very large. Equivalently we see that the credible interval of the random-walk smoking parameter is approximately linear. Comparing this with the DIC values for the two models we may hypothesize that they random-walk model will tend towards the linear model and that the true relationship is highly linear.  In any case, the inclusion of smoking in the model yields a better model.
