...
We then move on to $\kappa_v$. As for $\kappa_u$, we display both a plot of the whole chain and a plot where the first 500 values are removed.
...
Same as for the components of $\mathbf{u}$, the three components of $\boldsymbol \eta$ show varying degree of autocorrelation. The autocorrelations are comparable in size to the autocorrelations of $\mathbf{u}$, and much smaller than the autocorrelation of $\kappa_v$.
...
cat("ESS of kappa_v: ", effectiveSize(kappav))
```
The ESS is the estimated number of independent samples needed to obtain a parameter estimate with the same precision as the MCMC estimate based on $N$ dependent samples. In our case, $N=50000$, which means that the quality of our $\kappa_u$ sample is as good as `r round(effectiveSize(kappau))` independent samples. Also, our $\kappa_v$ sample is as good as `r round(effectiveSize(kappav))` independent samples, which is not very good. The $\kappa_u$ samples are better than the $\kappa_v$ samples, which also could be seen by the fact that the $\kappa_u$ samples are much less correlated than the $\kappa_v$ samples.

Because the parameters are correlated, we get high autocorrelation in the Markov chain, and hence a low value for the ESS. One way to improve this is by blocking. Blocking is done by simulating the variables together, and accepting or rejecting all of the new proposed variables simultaneously. SKRIV NOE OM METROPOLIS HASTINGS PROPOSAL DENSITY?!?!?!?!?!?!????!?!?
...
germany.plot(exp(result$summary.random$region$`0.5quant`) - post_median_u, col=col, legend=TRUE)
```
From figure \ref{fig:medianexpucomp}, we see that we generally have small differences between the computed $\exp(\mathbf{u})$ from MCMC and INLA, with some larger outliers. We observe no negative differences, KOMMENTER DETTE

```{r, echo=T, eval=T}
exp(result$summary.random$region$`0.5quant`[random_indexes])
post_median_u[random_indexes]
```
We observe that the MCMC and INLA estimates of selected components of $u$ are close.  

LEGG PÅ CAPTIONS OG DISKUTER FØLGENDE PLOTS.
```{r, echo=T, eval=T, fig.cap = "\\label{fig:marginals_hyperpar_kappau} The histogram of the MCMC samples in addition to the INLA marginal and improved INLA marginal of $\\kappa_u$."}
...
```{r, echo=T, eval=T, fig.cap = "\\label{fig:marginals_hyperpar_kappav} The histogram of the MCMC samples in addition to the INLA marginal and improved INLA marginal of $\\kappa_v$."}
...
```{r, echo=T, eval=T, fig.cap = "\\label{fig:marginals_u} The histogram of the MCMC samples in addition to the INLA marginal and improved INLA marginal of the selected components of $u$."}
...
ggplot() + geom_line(data=df, aes(x = x, y = y, col="INLA")) +  geom_line(data=df_imp, aes(x = x, y = y, col="INLA - improved")) + geom_histogram(data=df_samp, aes(x = x, y = ..density.., col="MCMC"), bins=100) + facet_wrap(~idx) + ylab("density") + xlim(-0.75, 0.75)
```


```{r, echo=T, eval=T, fig.cap = "\\label{fig:marginals_u} The histogram of the MCMC samples in addition to the INLA marginal and improved INLA marginal of the selected components of $\\boldsymbol \\eta$."}
inla_eta = result$marginals.fitted.values
inla_eta_improved = result_improved$marginals.fitted.values
...
ggplot() + geom_line(data=df, aes(x = x, y = y, col="INLA")) +  geom_line(data=df_imp, aes(x = x, y = y, col="INLA - improved")) + geom_histogram(data=df_samp, aes(x = x, y = ..density.., col="MCMC"), bins=100) + facet_wrap(~idx) + ylab("density")
```
...
```{r}
cat("model w/o smoking:", result$dic$dic, "\n")
cat("model w/  smoking lin:", result_2$dic$dic, "\n")
...
ggplot() + geom_ribbon(data=df70, aes(x = id, ymin=bot025,ymax=top975), alpha=0.50) +
  geom_line(data=df70, aes(x = id, y = median, col="mean"), size=1) +
  geom_line(data=df70, aes(x = id, y = bot025, col="bot025"), size = 1) + 
  geom_line(data=df70, aes(x = id, y = top975, col="top975"), size=1) + 
