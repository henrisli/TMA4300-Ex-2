...
We implement a Gibbs sampling algorithm with individual parameter updates using the full conditional for $\kappa_u$, $\kappa_v$ and $\mathbf{u}$. We update $\boldsymbol{\eta}$ using Metropolis-Hastings with $q(\boldsymbol{\eta} \lvert \mathbf{z}, \cdot)$ as the proposal density. We set $\mathbf{z} = \boldsymbol{\eta}^{(m-1)}$.
...
```
```{r}
df2 = data.frame(kappa_u = kappau[500:N_iter], N = 500:N_iter)
ggplot(df2, aes(x = N, y = kappa_u)) + geom_point()
```

```{r}
acf(kappau, lag.max=200)
```

```{r}
gw = geweke.diag(kappau)
cat("Geweke's Convergence Diagnostic for kappa_v \n")
cat("zscores:", gw$z, "\n")
cat("pvals:", 2*(1-pnorm(gw$z)))
```


```{r}
df3 = data.frame(kappa_v = kappav, N = 1:N_iter)
ggplot(df3, aes(x = N, y = kappa_v)) + geom_point()
```

```{r}
acf(kappav, lag.max=200)
```
...
```{r}
random_indexes = sample(1:length(Oral$Y), 3, replace=F)
cat("indexes: ", random_indexes)
df4 = data.frame()
for (i in 1:length(random_indexes)){
  index = random_indexes[i]
  df = data.frame(u = u[, index], N = 1:N_iter, shape = as.factor(i), col=as.factor(i))
  df4 = rbind(df4, df)
}
ggplot(df4, aes(x = N, y = u, shape = shape, col=col)) + geom_point() + scale_shape(solid=F)
```
...
```{r}
gw = geweke.diag(df5)
cat("Geweke's Convergence Diagnostic for u \n")
cat("zscores:", gw$z, "\n")
cat("pvals:", 2*(1-pnorm(gw$z)))
```
...
for (i in 1:length(random_indexes)){
  index = random_indexes[i]
  df = data.frame(eta = eta[, index], N = 1:N_iter, shape = as.factor(i), col=as.factor(i))
  df6 = rbind(df6, df)
}
ggplot(df6, aes(x = N, y = eta, shape = shape, col=col)) + geom_point() + scale_shape(solid=F)
```
...
```{r}
gw = geweke.diag(df7)
cat("Geweke's Convergence Diagnostic for eta \n")
cat("zscores:", gw$z, "\n")
cat("pvals:", 2*(1-pnorm(gw$z)))
```
...
post_mean_u = apply(u[1000:10000,], 2, mean)
