...
```
From the plot, we see that we can use only every $\sim 50$^th^ iteration to get a correlation which is not statistically significant. Taking into account the burn-in period, we would then only get about 1000 uncorrelated samples from our Markov chain of size $N=50000$.
...
dfinla$region = seq(1,length(Oral$Y))
...
# Oral$Y = Oral$E * eta
result <- inla(formula, family="poisson", E = dfinla$E, data=dfinla,
...
germany.plot(exp(result$summary.random$region$mean) - post_median_u, col=col, legend=TRUE)
```
From figure \ref{fig:medianexpucomp}, we see that we generally have small differences between the computed $\exp(\mathbf{u})$ from MCMC and INLA, with some larger outliers.

Now, we want to compare histograms of the MCMC-samples and the posterior marginals obtained by INLA for both precision parameters $\kappa_u$ and $\kappa_v$, and the same three randomly chosen components of $\mathbf{u}$ and $\boldsymbol \eta$. We also note that improved estimates of the posterior marginals for the precision parameters can be obtained by applying `inla.hyperpar(result)` on the original INLA results object.

```{r, echo=T, eval=T, out.width = "50%", fig.align = "center", fig.cap = "\\label{fig:compkappau}Comparison of computed densities by INLA and histogram of MCMC sample for $\\kappa_u$."}
...
```
We see from figure \ref{fig:compkappau} that all the three distributions are fairly similar. Still, the improved INLA estimate distribution is closer to the MCMC sample than the original INLA distribution. Also worth noting is that the MCMC sampler uses `r round(sum(times)/60)` minutes, while INLA uses `r cat(round(as.vector(result$cpu.used)[4]),1)` seconds to run. Because they obtain similar posterior marginals, we see that INLA is vastly superior because of the time consumption.
...
```
For the three components of $\mathbf{u}$ in figure \ref{fig:comp3u} we see that the improved INLA coincides with the original INLA. Also, the distribution from INLA seems to be shifted somewhat to the right compared to the histogram of the MCMC samples.
...
```
KOMMENTER LITT OM ETA NÅR VI HAR SIMULERT 50000 GANGER.
...
ggplot() + geom_ribbon(data=df70, aes(x = id, ymin=bot025,ymax=top975), alpha=0.50) +
  geom_line(data=df70, aes(x = id, y = mean, col="mean"), size=1) +
