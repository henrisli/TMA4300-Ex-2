...
post_median_u = apply(exp(u[1000:dim(u)[1],]), 2, median)
...
formula_1 <- Y~f(region,model="besag", graph = g, hyper=list(prec=kappauprior), constr=F) + 
            f(region2, model="iid", hyper=list(prec=kappavprior))

formula_0 <- Y~f(region,model="besag", graph = g, hyper=list(prec=kappauprior)) + 
            f(region2, model="iid", hyper=list(prec=kappavprior))
