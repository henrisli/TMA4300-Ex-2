...
In general, INLA and MCMC produce results that are at least qualitatively comparable and the resulting marginal densities perfectly overlap with the histograms of the MCMC samples.  Worth noting is that the MCMC sampler uses `r round(sum(times)/60)` minutes, while INLA uses `r INLA.time` seconds to run. Because they yield very similar results, we see that INLA is vastly superior if time consumption is a significant restriction. 
