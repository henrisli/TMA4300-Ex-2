...
sample_eta = function(kappa_v, u, z){
  ...
  A_arg_sampler = diag.spam(c_taylor + kappa_v)
...
log_full_conditional_eta = function(eta, kappa_v, u, log=T){
  # Corresponds to p(eta | ...) in the text
...
log_full_conditional_eta_approx = function(eta, z, u, kappa_v,  log=T){
  # Corresponds to q(eta | ...) in the text
...
```{r, echo = F, eval = T, out.width = "50%", fig.align = "center", fig.cap = "\\label{fig:acfkappav1}Autocorrelation plot of $\\kappa_v$ with max lag of 50."}
acf(kappav, lag.max = 50)
```
```{r, echo = F, eval = T, out.width = "50%", fig.align = "center", fig.cap = "\\label{fig:acfkappav2}Autocorrelation plot of $\\kappa_v$ with max lag of 1000."}
acf(kappav, lag.max = 1000)
```

From the plot in figure \ref{fig:acfkappav1} we see that the correlation between consecutive samples of $\kappa_v$ is very high. From the plot in figure \ref{fig:acfkappav2}, we observe that the lag required to get non-significant autocorrelation is several hundred. The samples of $\kappa_v$ being highly correlated is not ideal, as it will take the MCMC algorithm several iterations to generate the equivalent of an independent sample. 
...
```
```{r, echo = F, eval = T, out.width = "33%", fig.align = "center", fig.cap = "\\label{fig:acfu1}Autocorrelation plot for component 157 of $\\mathbf{u}$"}
df5 = data.frame(u = u[,random_indexes])
acf(u[,random_indexes[1]], lag.max = 200, main = paste0("ACF of u[",random_indexes[1],"]"))
```
```{r, echo = F, eval = T, out.width = "33%", fig.align = "center", fig.cap = "\\label{fig:acfu2}Autocorrelation plot for component 222 of $\\mathbf{u}$"}
acf(u[,random_indexes[2]], lag.max = 200, main = paste0("ACF of u[",random_indexes[2],"]"))
```
```{r, echo = F, eval = T, out.width = "33%", fig.align = "center", fig.cap = "\\label{fig:acfu3}Autocorrelation plot for component 429 of $\\mathbf{u}$"}
acf(u[,random_indexes[3]], lag.max = 200, main = paste0("ACF of u[",random_indexes[3],"]"))
```

From figure \ref{fig:acfu1}-\ref{fig:acfu3} we see that the three components of $\mathbf{u}$ show varying degree of autocorrelation. It takes roughly $50$ iterations to reduce the autocorrelation of the three components to non-significance. Even so, the components are less correlated than $\kappa_v$.
...
```{r, echo = T, eval = T, out.width = "50%", fig.align = "center", fig.cap = "\\label{fig:3v}Trace plot for three random components of $\\mathbf{v}$."}
df4 = data.frame()
for (i in 1:3){
  index = random_indexes[i]
  df = data.frame(v = eta[, index] - u[, index], N = 1:N_iter, shape = as.factor(i), col=as.factor(index))
  df4 = rbind(df4, df)
}
ggplot(df4, aes(x = N, y = v, shape = shape, col=col)) + geom_point() + scale_shape(solid=F)+ guides(shape = F) + ggtitle("Trace plot for v")
```
From the trace plots in figure \ref{fig:3v} of the three components of $\mathbf{v}$ we observe only a small burn-in period.
...
```{r, echo = F, eval = T, out.width = "33%", fig.align = "center", fig.cap = "\\label{fig:acfv1}Autocorrelation plot for component 157 of $\\mathbf{v}$"}
acf(eta[,random_indexes[1]]-u[,random_indexes[1]], lag.max = 200, main = paste0("ACF of v[",random_indexes[1],"]"))
```
```{r, echo = F, eval = T, out.width = "33%", fig.align = "center", fig.cap = "\\label{fig:acfv2}Autocorrelation plot for component 222 of $\\mathbf{v}$"}
acf(eta[,random_indexes[2]]-u[,random_indexes[2]], lag.max = 200, main = paste0("ACF of v[",random_indexes[2],"]"))
```
```{r, echo = F, eval = T, out.width = "33%", fig.align = "center", fig.cap = "\\label{fig:acfv3}Autocorrelation plot for component 429 of $\\mathbf{v}$"}
acf(eta[,random_indexes[3]]-u[,random_indexes[3]], lag.max = 200, main = paste0("ACF of v[",random_indexes[3],"]"))
```

From figures \ref{fig:acfv1}-\ref{fig:acfv3} we see that the three components of $\mathbf{v}$ show varying degree of autocorrelation. As it takes roughly $100$ iterations to reduce the autocorrelation to non-significance we observe that the three components are more autocorrelated than the components of $\mathbf{u}$, but still less correlated than $\kappa_v$.
...
cat("zscores:", gw$z, "\n")
cat("pvals:", 2*(1-pnorm(abs(gw$z))))
p_val1 = round(2*(1-pnorm(abs(gw$z)))[1],2)
p_val2 = round(2*(1-pnorm(abs(gw$z)))[2],2)
p_val3 = round(2*(1-pnorm(abs(gw$z)))[3],2)
```
$p$-values of `r p_val1`, `r p_val2` and `r p_val3` indicate that the chain converges during the first $10\%$ iterations for all three components in consideration.
...
To improve the ESS of our MCMC algorithm we need to decrease the autocorrelation of the samples in our algorithm. One method is to implement blocking, which is done by sampling some or all of the parameters together. In this case it would be natural to sample $\kappa_u$, $\mathbf{u}$, $\kappa_v$ and $\boldsymbol \eta$ all together, because $\mathbf{u}$ is dependent on both $\kappa_u$ and $\kappa_v$ and $\boldsymbol \eta$ is dependent on $\kappa_v$. We would then accept or reject $(\kappa_u, \mathbf{u}, \kappa_v, \boldsymbol \eta)$ as if it were a single sample. To implement this we would have to use the joint density of all the parameters. Naturally, sampling from these densities increases the complexity of the algorithm. Also, we could experience very low acceptance probabilities when accepting/rejecting all the parameters together, which would decrease the preformance of the algorithm.
...
```{r, echo = T, eval = T, out.width = "50%", fig.align="center", fig.cap = "\\label{fig:accept}Acceptance probabilities for sampling $\\boldsymbol \\eta$."}
df90 = data.frame(alpha = accept_prob, N = seq(1, N_iter))
ggplot(df90, aes(x = N, y = alpha, col="acceptance probability")) + geom_line() +geom_smooth(aes(col = "smooth approx")) + ylab("r")
```
...
formula <- Y~ - 1 + f(region,model="besag", graph = g, hyper=list(prec=kappauprior), constr=FALSE) + 
...
```{r, echo = T, eval = T, out.width = "50%", fig.align = "center", fig.cap = "\\label{fig:medianexpucomp} The difference between the MCMC estimate of the posterior median of $\\exp(\\mathbf{u})$ and the INLA estimate of $\\exp(\\mathbf{u})$ for all regions in Germany."}
germany.plot(exp(result$summary.random$region$`0.5quant`) - post_median_u, col=col, legend=TRUE)
```
From figure \ref{fig:medianexpucomp}, we see that we have reasonably small differences between the computed $\exp(\mathbf{u})$ from MCMC and INLA, with some larger outliers. However, we observe no negative differences, indicating that INLA might overestimate the effect of the spatial effect $\mathbf{u}$.  
...
```
For the three components of $\mathbf{u}$ in figure \ref{fig:comp3u} we see that the improved INLA marginals coincide with the original INLA marginals. The INLA marginals coincide with the histogram of the MCMC samples. 

```{r, echo = T, eval = T, out.width = "70%", fig.align = "center", fig.cap = "\\label{fig:comp3v}Comparison of computed densities by INLA and histogram of MCMC sample for three randomly chosen components of $\\mathbf{v}$."}
inla_v = result$marginals.random$region2
inla_v_improved = result$marginals.random$region2

index = random_indexes[1]
df1 = data.frame(inla_v[index], idx=index); colnames(df1) = c("x", "y", "idx")
df1_imp = data.frame(inla_v_improved[index], idx=index); colnames(df1_imp) = c("x", "y", "idx")
df1_samp = data.frame(x = eta[,index] - u[, index], idx=index)

index = random_indexes[2]
df2 = data.frame(inla_v[index], idx=index); colnames(df2) = c("x", "y", "idx")
df2_imp = data.frame(inla_v_improved[index], idx=index); colnames(df2_imp) = c("x", "y", "idx")
df2_samp = data.frame(x = eta[,index] - u[, index], idx=index)

index = random_indexes[3]
df3 = data.frame(inla_v[index], idx=index); colnames(df3) = c("x", "y", "idx")
df3_imp = data.frame(inla_v_improved[index], idx=index); colnames(df3_imp) = c("x", "y", "idx")
...
```
For the three components of $\mathbf{v}$ in figure \ref{fig:comp3v} we see that the improved INLA coincides with the original INLA. Also, the marginal densities from INLA coincide with the histograms of the MCMC samples.
...
We note that the INLA marginals of the three components of $\boldsymbol \eta = \mathbf{u} + \mathbf{v}$ perfectly overlap with the MCMC histograms, see figure \ref{fig:comp3eta}.

```{r, echo = F, eval = T}
INLA.time = round(result$cpu.used[4],1)
```

In general, INLA and MCMC produce results that are at least qualitatively comparable and the resulting marginal densities seem to overlap with the histograms of the MCMC samples. Noting that the MCMC sampler uses `r round(sum(times)/60)` minutes, while INLA uses `r INLA.time` seconds to run. Because they yield very similar results, we see that INLA is vastly superior if time consumption is a significant restriction. 
...
formula_2 <- Y~ - 1 + f(region,model="besag", graph = g, hyper=list(prec=kappauprior), constr=T) + 
            f(region2, model="iid", hyper=list(prec=kappavprior)) + 
...
formula_3 <-  Y~ - 1 + f(region,model="besag", graph = g, hyper=list(prec=kappauprior), constr=T) + 
            f(region2, model="iid", hyper=list(prec=kappavprior)) + 
            f(smoking, model="rw2")
