...
```{r, echo = F, eval = T, fig.align = "center", out.width = "80%"}
set.seed(123)
...
cat("pvals:", 2*(1-pnorm(abs(gw$z))))
```
A $p$-value of `r 2*(1-pnorm(abs(gw$z)))` indicates that the chain did (not) converge during the first $10\%$ iterations.
...
```
The trace plot for three components of $\mathbf{u}$ looks correct, with a small burn-in period.
...
cat("pvals:", 2*(1-pnorm(abs(gw$z))))
```
$p$-values of `r 2*(1-pnorm(abs(gw$z)))` indicates that the chain converges during the first $10\%$ iterations for only component 89, and not for component 98 and 253 (DISSE TALLENE MÅ ENDRES FOR Å MATCHE NY SEED).
...
Same as for the components of $\mathbf{u}$, the three components of $\boldsymbol \eta$ show varying degree of autocorrelation, with some being highly correlated and others almost uncorrelated (SKRIV OM BASERT PÅ NY SAMPLE).
...
SOME CONCLUDING REMARKS TO SUM IT ALL UP
...
cat("Relative ESS of kappa v: ", effectiveSize(kappav) / running_time)
```
This means that the MCMC algorithm produces the equivalent of `r effectiveSize(kappau) / running_time` independent samples for $\kappa_u$ per second, and the equivalent of `r effectiveSize(kappav) / running_time` independent samples for $\kappa_v$ per second.
...
# Look at results
res$summary.random$region

res2 = inla.hyperpar(res)

germany.plot(exp(res$summary.random$region$mean), col=col, legend=TRUE)
germany.plot(post_median_eta, col=col, legend=TRUE)


germany.plot( exp(res$summary.random$region$mean) - post_median_eta, col=col, legend=TRUE)
```

```{r}
exp(res$summary.random$region$mean[random_indexes])
post_median_eta[random_indexes]
```

```{r}
df60 = data.frame(res2$marginals.hyperpar$`Precision for region`)
...
```{r}
df60 = data.frame(res2$marginals.hyperpar$`Precision for region2`)
df61 = data.frame(x = kappav[1000:length(kappav)])
