...
p1 = ggplot(df1, aes(x = N, y = kappa_u)) + geom_point()
df12 = data.frame(kappa_u = kappau[500:N_iter], N = 500:N_iter)
p2 = ggplot(df12, aes(x = N, y = kappa_u)) + geom_point()
...
```
From the plot, we see that we can use only every $\sim 45$^th^ iteration to get a correlation which is not statistically significant. Taking into account the burn-in period, we would then only get about 200 uncorrelated samples from our Markov chain of size $N=10000$.
...
cat("pvals:", 2*(1-pnorm(abs(gw$z))))
```
$p$-values of x indicates that the chain converges during the first $10\%$ iterations for only component 89, and not for component 98 and 253 (DISSE TALLENE MÅ ENDRES FOR Å MATCHE NY SEED).
...
germany.plot( exp(res$summary.random$region$mean) - post_median_u, col=col, legend=TRUE)
```
