...
From the plot in figure \ref{fig:acfkappav} we see that the correlation between consecutive samples of $\kappa_v$ is very high. From We observe that the lag required to get non-significant autocorrelation is several hundred. That the samples of $\kappa_v$ are highly correlated is not ideal, as it will take the MCMC algorithm several iterations to generate the equivalent of an independent sample. 
...
cat("model with smoking as linear component:", result_2$dic$dic, "\n")
cat("model with smoking as RW2:", result_3$dic$dic)
difference = result_2$dic$dic-result_3$dic$dic
```
