...
```{r, echo = F, eval = T, out.width = "50%", fig.align="center", fig.cap = "\\label{fig:accept}Acceptance probabilities for sampling $\\boldsymbol \\eta$."}
df90 = data.frame(alpha = accept_prob, N = seq(1, N_iter))
ggplot(df90, aes(x = N, y = alpha, col="acceptance probability")) + geom_line() +geom_smooth(aes(col = "loess approximation"), method="loess") + ylab("r")
```
...
ggplot() + geom_histogram(data=df_samp, aes(x = x, y = ..density.., col="MCMC"), bins=100) + geom_line(data=df, aes(x = x, y = y, col="INLA")) +  geom_line(data=df_imp, aes(x = x, y = y, col="INLA - improved")) + facet_wrap(~idx) + ylab("density")
```
We note that even though INLA gives positively shifted estimates for $\mathbf{u}$, the INLA marginals of the three components of $\boldsymbol \eta = \mathbf{u} + \mathbf{v}$ perfectly overlap with the MCMC histograms, see figure \ref{fig:comp3eta}.

```{r, echo = F, eval = T}
INLA.time = round(result$cpu.used[4],1)
```
In general, INLA and MCMC produce results that are at least qualitatively comparable. For $\kappa_u$, $\kappa_v$ and $\boldsymbol \eta$, the resulting marginal densities perfectly overlap. For the spatial effect $\mathbf{u}$ we observe some positive shift in the INLA estimate.  Worth noting is that the MCMC sampler uses `r round(sum(times)/60)` minutes, while INLA uses `r INLA.time` seconds to run. Because they yield very similar results, we see that INLA is vastly superior if time consumption is a significant restriction. 
